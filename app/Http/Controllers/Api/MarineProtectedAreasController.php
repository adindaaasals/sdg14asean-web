...
class MarineProtectedAreasController extends Controller
{
    ...
    public function uploadPolygonFile(Request $request)
    {
        // Validasi file
        $validated = $request->validate([
            'polygon_file' => 'required|file|mimes:json|max:204800', // Max 200MB
        ]);

        // Simpan file ke storage dan database
        $file = $request->file('polygon_file');
        $filename = time() . '-' . $file->getClientOriginalName();
        $path = $file->storeAs('mpa_data', $filename, 'public');

        // Simpan path ke database
        $marineProtectedArea = new MarineProtectedAreas();
        $marineProtectedArea->polygon_data = $path;
        $marineProtectedArea->save();
