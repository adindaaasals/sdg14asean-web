...
class AquacultureProductionController extends Controller
{
    ...
    public function productionTrend($countryId)
    {
        $data = AquacultureProduction::query()
            ->where('id', $countryId)
            ->select(
                DB::raw('SUM(aquaculture_production_2020) as production_2020'),
                DB::raw('SUM(aquaculture_production_2021) as production_2021'),
                DB::raw('SUM(aquaculture_production_2022) as production_2022')
            )
            ->first();
        ...
        return response()->json([
            ...
            'values' => [
                $data->production_2020,
                $data->production_2021,
                $data->production_2022,
            ],
        ]);
    }
}
